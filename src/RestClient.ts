...
export class RestClient {
	...
	private _clockSkewOffset: number = 0;
	private _shouldApplyClockSkewOffset: boolean = false;
	/**
...
*/
	...
	private _signed(params, credentials, isAllResponse) {
		...
		const clockSkewOffset = this._shouldApplyClockSkewOffset
		...
		return axios(signed_params)
			...
			.catch(error => {
				if (this._handleSignatureError(error)) {
					this._calculateClockSkewOffset(error);
	...
	private _calculateClockSkewOffset(response: AxiosResponse) {
	...
	private _handleSignatureError(response: AxiosResponse) {
		let errorCode = response.headers
